...
function main(): void {
    ...
    function writeFile(fileName: string, contents: string) {
        fs.writeFile(
            path.join(path.dirname(inputFilePath), fileName),
            contents,
            { encoding: "utf-8" },
            (err) => {
                if (err) throw err;
    ...
    const diagnosticMessagesJson: { [key: string]: DiagnosticDetails } =
        JSON.parse(inputStr);
...
function buildInfoFileOutput(
    messageTable: InputDiagnosticMessageTable,
    inputFilePathRel: string,
    thisFilePathRel: string
): string {
    let result =
        "// <auto-generated />\r\n" +
        "// generated from '" +
    ...
    messageTable.forEach(
        (
            ...
            const argReportsUnnecessary = reportsUnnecessary
                ...
                : "";
            const argElidedInCompatibilityPyramid = elidedInCompatibilityPyramid
                ? `${
                      !reportsUnnecessary
                          ? ", /*reportsUnnecessary*/ undefined"
                          : ""
                  }, /*elidedInCompatibilityPyramid*/ ${elidedInCompatibilityPyramid}`
                : "";
            const argReportsDeprecated = reportsDeprecated
                ? `${
                      !argElidedInCompatibilityPyramid
                          ? ", /*reportsUnnecessary*/ undefined, /*elidedInCompatibilityPyramid*/ undefined"
                          : ""
                  }, /*reportsDeprecated*/ ${reportsDeprecated}`
                : "";

            result += `        ${propName}: diag(${code}, DiagnosticCategory.${category}, "${createKey(
                propName,
                code
            )}", ${JSON.stringify(
                name
            )}${argReportsUnnecessary}${argElidedInCompatibilityPyramid}${argReportsDeprecated}),\r\n`;
        }
    );
...
function buildDiagnosticMessageOutput(
    messageTable: InputDiagnosticMessageTable
): string {
    let result = "{";
